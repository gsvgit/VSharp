...
type ShortestDistanceWeighter(target : codeLocation) =
    ...
    let preTargetWeight currLoc =
        ...
        let targets =
            ...
            |> Seq.map (fun kv -> { offset = (localCFG.ResolveBasicBlock kv.Key).StartOffset; method = currLoc.method })
    ...
    let postTargetWeight currLoc =
        let localCFG = currLoc.method.CFG
        let targets = localCFG.Sinks |> Seq.map (fun offset -> { offset = offset.StartOffset; method = currLoc.method })
        localWeight currLoc targets |> Option.map ((+) 32u)
