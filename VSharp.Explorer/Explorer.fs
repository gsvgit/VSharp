...
type EntryPointConfiguration(mainArguments: string[]) =
    ...
    member x.Args =
        if mainArguments = null then None else Some mainArguments
...
type private SVMExplorer(explorationOptions: ExplorationOptions, statistics: SVMStatistics, reporter: IReporter) =
    ...
    let mutable isCoverageAchieved: unit -> bool =
        always false
    ...
    let rec mkForwardSearcher mode =
        ...
        | InterleavedMode (base1, stepCount1, base2, stepCount2) ->
            InterleavedSearcher (
                [
                    mkForwardSearcher base1, stepCount1
                    mkForwardSearcher base2, stepCount2
                ]
            )
            :> IForwardSearcher
            ...
            let this =
                if method.HasThis then
                    if Types.IsValueType declaringType then
                        ...
                        let this =
                            Memory.MakeSymbolicThis initialState method
    ...
    member private x.FormEntryPointInitialStates
        ...
        try
            ...
            | Some args ->
                ...
                let args =
                    Some (List.singleton (Some argsRef))
            ...
            | None when hasParameters ->
                ...
                let argsParameterTerm =
                    Memory.ReadArgument state argsParameter
...
type private FuzzerExplorer(explorationOptions: ExplorationOptions, statistics: SVMStatistics) =
    ...
    interface IExplorer with
        ...
        member x.StartExploration isolated _ =
            ...
            let outputDir =
                explorationOptions.outputDirectory.FullName
...
type public Explorer(options: ExplorationOptions, reporter: IReporter) =
    ...
    let statistics =
        new SVMStatistics (Seq.empty, true)
        ...
        try
            if method.ContainsGenericParameters then
                ...
                | Some (classParams, methodParams) ->
                    let classParams =
                        classParams |> Array.choose getConcreteType
                    let methodParams =
    ...
    member x.StartExploration
        ...
        try
            ...
            let finished =
                Task.WaitAll (explorationTasks, options.timeout)
