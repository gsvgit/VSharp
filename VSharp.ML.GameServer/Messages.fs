...
type OutgoingMessage =
    | GameOver of uint<percent> * uint32<test> * uint32<error> * uint32<step>
    | MoveReward of Reward
    | IncorrectPredictedStateId of uint<stateId>
...
let deserializeInputMessage (messageData: byte[]) =
    let rawInputMessage =
        let str =
            Encoding.UTF8.GetString messageData
...
let serializeOutgoingMessage (message: OutgoingMessage) =
    match message with
    | GameOver (actualCoverage, testsCount, errorsCount, stepsCount) ->
        RawOutgoingMessage ("GameOver", box (GameOverMessageBody (actualCoverage, testsCount, errorsCount, stepsCount)))
