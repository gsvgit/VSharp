...
// ------------------------------- Encoding: common -------------------------------
        ...
        member public x.EncodeTerm encCtx (t : term) : encodingResult =
            let getResult () =
                ...
                let symbolicType = TypeOf t
                if Types.IsEnum symbolicType then x.AddEnumAssumptions encCtx symbolicType result
                else result
            encodingCache.Get(t, getResult)

        member private x.AddEnumAssumptions encCtx symbolicType (encodingResult : encodingResult) =
            let typ = Types.ToDotNetType symbolicType
            assert(typ.IsEnum)
            ...
            let createAssumption assumptions value =
                let assumptions', concrete = x.EncodeConcrete encCtx value symbolicType |> toTuple
